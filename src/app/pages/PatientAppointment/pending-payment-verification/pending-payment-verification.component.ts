...
export class PendingPaymentVerificationComponent implements OnInit {
  ...
  async getPaymentStatus() {
    ...
    if (orderId) {
      try {
        const response = await this.service.post({ orderId }, "/api/v1/payment/payment-verify").toPromise();
        // console.log("PaymentVerify=>", response);
    
        if (response.data.order_status === "PAID") {
          await this.paymentUpdateForCashFree(response?.data.cf_order_id,orderId);
